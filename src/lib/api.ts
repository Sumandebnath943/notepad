/**
 * Thin browser API client. All calls go to our own Next.js Route Handlers
 * (same-origin), which hold the httpOnly Supabase session and talk to Supabase
 * server-side. The browser never sees the auth token and never talks to
 * Supabase directly.
 *
 * Ciphertext is fine to send here; the master password / keys are NOT.
 */
